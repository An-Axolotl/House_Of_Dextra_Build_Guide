...
def parse_args():
    ...
    parser.add_argument('--config', '-c',
                       ...
                       help='Path to configuration file (default: config/anthro_standard.yaml)')
...
def build_obs_mj(qpos_sim_mj: np.ndarray, cur_targets_sim_mj: np.ndarray) -> torch.Tensor:
    """
    Build observation for 5-finger hand (no morphology conditioning needed).
    
    Observation structure:
    - Normalized joint positions (20 values): [-1, 1] range
    - Target joint positions (20 values): raw sim-rad (unnormalized)
    - History: above 40 values over multiple frames
    - Object one-hot encoding: which object is being manipulated
    
    Returns:
        torch.Tensor: Observation vector of shape (1, obs_dim)
    """
    ...
    hist_buf[-1] = frame
    flat = hist_buf.reshape(-1)  # (history_frames * 40,)
    
    # Concatenate: history buffer + object one-hot (no morphology condition needed for 5-finger)
